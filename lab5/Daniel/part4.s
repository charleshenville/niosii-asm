...
SKIP_EA_DEC:
	...
	subi sp,sp,12 #push r8,r9 onto stack
	stw r8,(sp)
	stw r9,4(sp)
	stw r10,8(sp)
	
	movia r4,KEYs #load address of keys, timer and RUN as parameter
	movia r5,TIMER
	movia r6, RUN
...
_start:
	...
	movia r8,COUNT	#reset count
	stw r0,(r8)
	
	movia r8,RUN #reset RUN
	stw r0,(r8)
...
KEY_ISR:
	ldwio r8,0xC(r4) #load edge captures into r8
	beq r8,r0,done #check empty edge capture
	
	
	srli r8,r8,1 #go to key 1 functionality if key 0 not pressed
	bne r8,r0,KEY1
	
	ldw r9,(r6) #flip RUN bit
	xori r9,r9,1
	stw r9,(r6)
	
	movia r8,1 #reset edge cap
	stwio r8,0xC(r4)
	br done
	
KEY1:
	srli r8,r8,1 #go to key 2 functionality if key 1 not pressed
	bne r8,r0,KEY2
	
	ldwio r9,0xC(r5)#stores the current timer count value in r9
	slli r9,r9,16
	ldwio r10,8(r5)
	add r9,r9,r10
	
	srli r9,r9,1 #halves the timer count value therefore doubling freq
	
	movia r10,0b1011 #pauses timer
	stwio r10,4(r5)
	
	srli r10,r9,16 #store upper 16 bits of timer count
	stwio r10,0xC(r5)
	
	andi r10,r9,0xffff #store lower 16 bits of timer count
	stwio r10,8(r5)
	
	movia r10, 0b0111 #set timer to start, cont, and allow interrupts
	stwio r10,4(r5) 
	
	movia r8,0b10 #reset edge cap
	stwio r8,0xC(r4)
	br done
KEY2:
	srli r8,r8,1 #go to key 3 if key 2 not pressed
	bne r8,r0,KEY3
	...
	slli r9,r9,1 #doubles the timer count value therefore halving freq
	...
	movia r8,0b100 #reset edge cap
	stwio r8,0xC(r4)
	br done
	
KEY3:
	stwio r0,0xC(r4) #clear edge cap
done: ret
